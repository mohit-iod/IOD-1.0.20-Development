...
@interface PatientVideoCallViewController ()<OTSessionDelegate, OTPublisherDelegate,OTSubscriberDelegate,UITextFieldDelegate> {
...
@implementation PatientVideoCallViewController
-(void)setUI {
...
- (void)sessionDidConnect:(OTSession*)session
{
    ...
    if([doctorService.isCallInterrupted isEqualToString:@"yes"]){
        ...
        timerForWaiting = [NSTimer scheduledTimerWithTimeInterval:70
                                                           target:self
...
streamDestroyed:(OTStream *)stream
{
    ...
    doctorService.visit_id = patientService.res_visit_id;
    doctorService.InterruptionTime = currMinute;
    
    doctorService.callInterruptionMessage =@"Call interrupted by doctor but patient made it success";
    if(isCallEnd) {
        [countdownTimer invalidate];
        ...
        CallInterruptionViewController *interruptionVC =[self.storyboard instantiateViewControllerWithIdentifier:@"CallInterruptionPatient"];
        [self.navigationController pushViewController:interruptionVC animated:YES];
    }
}
...
- (void)subscriberDidConnectToStream:(OTSubscriberKit *)subscriber {
    ...
    if(![doctorService.isCallInterrupted isEqualToString:@"yes"]){
        ...
        if ([patientService.visit_type_id isEqualToString:@"2" ]|| [patientService.visit_type_id isEqualToString:@"3"]){
            //No payment for Book appointment and second opinion.
        }
        
        else if([patientService.visit_type_id isEqualToString:@"1"]){
        ...
        else if([patientService.visit_type_id isEqualToString:@"1"] &&  patientService.payment_mode_id == 4){
            [self creditCardPayment];
        }
        else{
            [self postPaymentInfo];
...
//
//-(IBAction)OpenDocumentView:(UIButton *)sender {
//    [_documentView setHidden:NO];
//}
//
//-(IBAction)closeDocumentView:(UIButton *)sender {
//    [_documentView setHidden:YES];
//    // [self.view bringSubviewToFront:_subscriber.view];
...
- (void) postPaymentInfo {
    ...
    NSString *strPaymentMode = [NSString stringWithFormat:@"%d",patientService.payment_mode_id];
    ...
    [parameter setObject:strPaymentMode forKey:@"payment_mode_id"];
    
    
    NSLog(@"post payment parameters %@", parameter);
